...
### 模式 D：Hook 层 fail-open 架构（批注后新增）

Baton 的 hook 层整体采用 fail-open 设计：write-lock.sh 在无法确定 target path、找不到 _common.sh、遇到未预期错误时均 exit 0 放行；BATON_BYPASS=1 可以绕过全部检查；markdown 无条件放行。这是有意的架构选择（宿主 hook 生态不稳定，fail-open 优先可用性），但意味着 Baton 的安全保证是"常态路径强约束，异常路径不误伤"，而非绝对 fail-closed。B7（markdown 穿透）和 B12（phase boundary 穿透）都是这一模式的实例。

### 模式 E：治理事实多副本漂移（批注后新增）

同一治理事实同时出现在 constitution、shared-protocols、phase skills、phase-guide fallback 文本、parser/hooks、review prompt/templates 六处，靠人工同步。审计中的 C1（失败阈值）、C2（FINISH 步骤）、C8（spike vs no-source-changes）、C9（Annotation Log）、C11（Todo schema）均属于此模式的实例。Constitution 的 Authority Model 定义了冲突解决优先级，但没有为每个治理事实指定 authoritative source——冲突只有在 AI 运行时同时看到两个矛盾版本时才可能被发现，而这依赖 AI 的注意力和上下文加载顺序。此前的模式 C（Phase-guide vs phase skill 不一致）是本模式的一个高频子类。

## 批注区

> 我认为审计还漏掉了什么
>
> 这里是我和 skill-audit.md 的差异点。不是说它错，而是它主要盯着 skill/protocol 语义，没有把运行时硬化层看全。

### [Annotation 1] 系统默认安全姿态是 fail-open

- **Trigger / 触发点**: 批注指出 write-lock.sh 在多种情况下 fail-open 放行——拿不到 target path、找不到 _common.sh、BATON_BYPASS=1、markdown 永远放行。认为这比审计文档中 B7（"Markdown 写入可修改关键标记"）更本质，因为涉及系统整体的默认安全姿态。
- **Intent as understood / 理解后的意图**: 审计只在 B7 讨论了 "markdown 穿透" 这一个 fail-open 案例，但没有上升到系统级别审视 Baton 的默认安全姿态。真正的发现应该是：Baton 的 hook 层整体采用 fail-open 设计——这是一个有意的架构选择（宿主 hook 生态不稳定），但审计应当把这作为系统性发现而非散点问题。
- **Response / 回应**:

  验证了全部 fail-open 路径 [CODE] ✅：
  - `write-lock.sh:14` — trap handler: unexpected error → exit 0（fail-open）
  - `write-lock.sh:17-20` — BATON_BYPASS=1 → exit 0
  - `write-lock.sh:49-55` — 无法确定 target path → exit 0
  - `write-lock.sh:57-60` — markdown 文件 → 无条件 exit 0
  - `write-lock.sh:64-69` — 找不到 _common.sh → exit 0

  批注判断正确。审计在"模式 A：规则层定义了但 hook 层不执行"中触及了这个方向，但停在了"哪些规则有/没有硬执行"的枚举层面，没有提炼出更根本的结论：**Baton 的 hook 层是 fail-open 架构，设计意图是"常态路径强约束，异常路径不误伤"**。这应该作为第四部分（系统性模式）的一个独立模式，而非隐含在模式 A 中。

  具体而言，模式 A 说"规则层定义了但 hook 层不执行"，这暗示 hook 层"应该执行但没做到"。批注的洞察更准确：hook 层是**有意**选择 fail-open，问题不是缺失而是设计权衡。这改变了修复方向——不是"给所有规则加 hook"，而是"明确哪些规则 fail-open 是可接受的，哪些需要升级到 fail-closed"。

- **Status**: ✅ accepted
- **Impact**: affects conclusions — 第四部分已新增模式 D："Hook 层 fail-open 架构"，将 B7 重新归类为该模式的实例。模式 A 的措辞需后续修正，区分"有意 fail-open"和"规则缺失 hook"。

---

### [Annotation 2] Write set "硬边界"在 hook 层是软的

- **Trigger / 触发点**: 批注指出 constitution 把 approved write set 写成 "hard boundary"（Core Invariant 4），但 post-write-tracker.sh 自己注释写了 "Advisory only"，始终 exit 0，只做警告。真正硬拦的是"没 GO 不能写源码"，不是"有 GO 但超写集不能写"。系统更像"入口硬门 + 执行期软治理"。
- **Intent as understood / 理解后的意图**: 审计在 B7 和模式 A 中分别提到了 write set 警告和规则/hook 不对齐，但没有把 constitution 的 "hard boundary" 承诺与 hook 层的实际执行力做直接对比。这暴露了 constitution 用词与系统实际保证之间的落差。
- **Response / 回应**:

  验证了两端证据 [CODE] ✅：
  - `constitution.md:28-29` — "Approved scope is a hard boundary, not a suggestion."
  - `constitution.md:115-117` — "The approved write set defines the files implementation is authorized to modify."
  - `post-write-tracker.sh:2-6` — "Advisory: warn when modified files aren't in the plan... Always exit 0 — PostToolUse cannot block, this is advisory only"
  - `post-write-tracker.sh:12` — trap handler 也 exit 0
  - `post-write-tracker.sh:98` — 最终无条件 exit 0

  批注判断正确。当前的执行保证层次是：

  | 层次 | 保证 | 硬度 |
  |------|------|------|
  | 没有 plan + GO → 不能写源码 | write-lock.sh | 硬（fail-open 例外见 Annotation 1） |
  | 有 GO 但超出 write set | post-write-tracker.sh | 软（advisory only，exit 0） |
  | Write set 扩展需要 plan revision | constitution 规则 | 纯规则，无 hook |

  Write set 约束实际上是两阶段的："入口硬门"（write-lock）+ "执行期软治理"（post-write-tracker）。Constitution 说 "hard boundary" 但 hook 层只能做到 "soft advisory"——这是 constitution 承诺与系统能力之间的落差。

  与 B7 的区别：B7 讨论 markdown 穿透（AI 可修改标记），这里讨论**源码** write set 超出（AI 写了不在 write set 中的 .ts/.js 文件，只会收到警告而非阻断）。应作为独立发现。

- **Status**: ✅ accepted
- **Impact**: affects conclusions — 第三部分严重度汇总需新增中严重度发现 B17（constitution "hard boundary" 承诺与 post-write-tracker advisory-only 实现之间的落差）。

---

### [Annotation 3] 治理事实无单一真相源，靠人工同步

- **Trigger / 触发点**: 批注指出同一治理事实同时出现在 constitution、shared-protocols、phase skills、phase-guide fallback 文本、parser/hooks、review prompt/templates 六处，靠人工同步。
- **Intent as understood / 理解后的意图**: 审计发现了多处具体矛盾（C1、C2、C8 等），但没有归因到根本原因——同一治理事实的多副本缺乏单一真相源（Single Source of Truth），导致副本之间不可避免地漂移。这是比任何单个矛盾都更根本的系统性问题。
- **Response / 回应**:

  回溯审计中的矛盾发现，确认多数属于"同一事实多副本漂移" [CODE] ✅：

  | 审计发现 | 漂移的治理事实 | 涉及的副本 |
  |---------|--------------|-----------|
  | C1 | 失败阈值 | constitution(:172) vs implement(:65) vs debug(:84) vs failure-tracker(:55) vs phase-guide(:159) |
  | C2 | FINISH 阶段步骤 | phase-guide.sh vs baton-implement Step 5 |
  | C8 | Research 阶段可执行操作 | baton-research Iron Law vs phase-guide |
  | C9 | Annotation Log 定义 | 6 处引用，0 处定义 |
  | C11 | Todo schema | baton-implement vs baton-plan vs baton-subagent vs plan-parser |

  批注判断正确。Constitution 的 Authority Model 定义了层级优先级（human > constitution > phase skills > extension skills），但这是**冲突解决规则**，不是**真相源指定**。每一层仍然各自陈述规则，冲突只有在 AI 运行时同时看到两个矛盾版本时才可能被发现——而这依赖 AI 的注意力和上下文加载顺序。

  此前的模式 C（Phase-guide vs phase skill 不一致）是本模式的一个高频子类。

- **Status**: ✅ accepted
- **Impact**: affects conclusions — 第四部分已新增模式 E："治理事实多副本漂移（无 Single Source of Truth）"，将模式 C 重新定位为该模式的子类。后续修复方向：为每个治理事实指定 authoritative source，其他层通过引用而非复述来获取。
