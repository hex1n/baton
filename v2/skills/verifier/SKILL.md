...
**Execution mode** (passed by Dispatch): Standard or Full.
- **Standard:** execute only this file.
- **Full:** execute this file, then read and execute module files listed by Dispatch.
...
Read brief.md § Approach and challenge on four dimensions:
...
1. **Prioritize independent evidence (L1 > L2 > L3).** In Mode A/B: do not read production code — use test results and runtime behavior. In Mode C: same-model code review is permitted but eval.md must declare "⚠️ Independence: degraded." Read tests (L2) to check quality in all modes.
...
5. **Be specific in findings.** "Data might be wrong" is useless. "threshold=0 passes validation but AC-1.1 says threshold > 0" is actionable.
6. **Credit what works.** Don't only flag problems. The human needs to know what's solid too.
7. **Max 5 pre-flight challenges.** Focus on highest impact. If the plan is good, say so.
8. **State your verification mode.** Every eval.md must say which mode (A/B/C) was used and what that means for confidence.
