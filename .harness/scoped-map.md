...
- 范围内:
  ...
  - `module-status.sh` 添加 `module_status_set_eval_round()` 写函数
...
- 预期写入边界:
  ...
  - `spec/bootstrap/module-status.sh` (添加写函数 + read_profile_value)
...
- 主要入口类或文件:
  ...
  4. **`spec/bootstrap/module-status.sh`** (220 行) -- module-status.md 解析器，当前只有读函数，无写函数
...
- 涉及的方法、API、命令或脚本:
  - `module_status_current_field()` -- 行级字段读取（state, owner, eval_round 等）
  - `module_status_rows_tsv()` / `module_status_current_row_tsv()` -- 行解析器
...
- 这些入口为什么相关:
  ...
  - Phase 2 的 6 项 gap 各自触及不同的 bootstrap 脚本或 hook 入口：eval round 触及 module-status.sh + SubagentStop hook、blocked 分类触及 PreToolUse hook、generator-feedback 触及 validate-artifact.sh + templates、retrospective 触及 validate-state-artifacts.sh、human gate 触及 PreToolUse hook、overlay 触及 harness-context.sh + explorer skill
...
Phase 1 — Hook 重构:
  ...
  hooks/pre-transition.sh (PreToolUse)
    ...
    -> parse-input.sh -> module-status.sh -> validate-transition.sh
  ...
  hooks/subagent-stop.sh (SubagentStop, CC-only)
    ...
    -> [新] eval round 自增 via module_status_set_eval_round()
...
Phase 2 — Enforcement Gap 调用链:
  ...
  Gap 1 (eval round 自增):
    SubagentStop hook -> hooks/subagent-stop.sh
      -> module-status.sh::module_status_set_eval_round()  [新写函数]
  ...
  Gap 5 (human gate advisory):
    PreToolUse hook -> hooks/pre-transition.sh
      ...
      -> to_state != blocked
      -> 读磁盘 module-status.md -> 检查 "- human_ack: true" under ## State Notes
      -> 未找到 -> hook_block()
...
- 当前可观察行为:
  ...
  - **module-status.sh**: 纯读取库，支持 `schema`/`rows`/`current-field`/`row-count`/`non-complete-count`。无任何写操作函数
  ...
  - **validate-artifact.sh**: 支持 6 种 artifact type（scoped-map, requirements, architecture, verification-path, evaluation, module-status），不支持 `generator-feedback`
...
- 当前校验规则:
  ...
  - 一致性: `check-consistency.sh` 有 11 个不变量（owners.txt 同步、states.txt 同步、template header 匹配、skills 同步、README 双语、governance 同步、agents 同步、symlink 相对路径、live module-status schema、isolation templates、provenance contract）
...
- 直接相关的测试:
  ...
  2. **`tests/test-module-status.sh`** -- module-status.sh 解析器测试。需要扩展以覆盖新写函数 `module_status_set_eval_round()`
...
- 这次改动是否可能触及迁移或 schema?
  - `module-status.md` 的 schema 不变（已有 `Eval Round` 列），但引入了首个写操作（`module_status_set_eval_round()`）
...
- **高风险区域**:
  ...
  3. **sed in-place ack 清除**: 如果 sed 表达式匹配错误行，会破坏 module-status.md 结构。应考虑使用 module-status.sh 写函数
...
- 预计文件数:
  ...
  - 修改: 10-12 个文件（install-hooks.sh, module-status.sh, validate-artifact.sh, validate-state-artifacts.sh, harness-context.sh, check-consistency.sh, profile.local.template.yaml, 2 技能文件, 3-5 现有测试文件）
...
- `post-artifact.sh` 的 ack 清除使用 sed in-place 编辑，是否应改用 module-status.sh 的写函数以保持一致性？CEO 计划指定 sed，但 eval round 使用写函数
