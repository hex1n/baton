...
make_repo "$tmp/cc-ok"
write_task_status "$tmp/cc-ok/.harness/task-status.md" "verification_check" "active" "- human_ack: true"
cc_payload='{"tool_input":{"file_path":".harness/task-status.md"}}'
...
TOTAL=$((TOTAL + 1))
if ! grep -q '^[-] human_ack: true$' "$tmp/cc-ok/.harness/task-status.md" 2>/dev/null; then
...
cat > "$tmp/cc-bad/.harness/task-status.md" <<'EOF'
# Task Status
...
- human_ack: true
EOF
bad_cc_payload='{"tool_input":{"file_path":".harness/task-status.md"}}'
assert_exit "invalid task-status does not clear ack" 2 run_hook "$tmp/cc-bad" "$bad_cc_payload"
TOTAL=$((TOTAL + 1))
if grep -q '^[-] human_ack: true$' "$tmp/cc-bad/.harness/task-status.md" 2>/dev/null; then
...
write_task_status "$tmp/cc-no-transition/.harness/task-status.md" "reviewing" "active" "- human_ack: true"
no_transition_payload='{"tool_input":{"file_path":".harness/task-status.md"}}'
run_hook "$tmp/cc-no-transition" "$no_transition_payload"
TOTAL=$((TOTAL + 1))
if grep -q '^[-] human_ack: true$' "$tmp/cc-no-transition/.harness/task-status.md" 2>/dev/null; then
...
write_task_status "$tmp/codex-bad/.harness/task-status.md" "blocked" "missing blocker" "[design_blocker]"
codex_payload="$(jq -n --arg command 'echo "| runtime-enforcement-hardening | generator | blocked | 0 | 2026-03-29T22:00:00+0800 | missing blocker |" > .harness/task-status.md' '{tool_input:{command:$command}}')"
