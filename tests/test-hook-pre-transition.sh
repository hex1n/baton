...
write_status "$tmp/cc-blocked-ok/.harness/task-status.md" "generating" "-"
cc_blocked_ok_payload="$(jq -n --arg content $'# Task Status\n\n| Scope | Owner | State | Eval Round | Updated At | Notes |\n|------|------|------|-----------|-----------|------|\n| runtime-enforcement-hardening | generator | blocked | 0 | 2026-03-29T22:00:00+0800 | [design_blocker] design mismatch |\n\n## State Notes\n- human_ack: true\n' '{tool_input:{file_path:".harness/task-status.md",content:$content}}')"
...
write_status "$tmp/cc-human-block/.harness/task-status.md" "awaiting_human_arch" "- human_ack: false"
human_payload="$(jq -n --arg content $'# Task Status\n\n| Scope | Owner | State | Eval Round | Updated At | Notes |\n|------|------|------|-----------|-----------|------|\n| runtime-enforcement-hardening | generator | verification_check | 0 | 2026-03-29T22:00:00+0800 | - |\n\n## State Notes\n- human_ack: false\n' '{tool_input:{file_path:".harness/task-status.md",content:$content}}')"
assert_exit "human gate blocks without ack" 2 run_hook "$tmp/cc-human-block" "$human_payload"

make_repo "$tmp/cc-human-ok"
write_status "$tmp/cc-human-ok/.harness/task-status.md" "awaiting_human_arch" "- human_ack: true"
human_ok_payload="$(jq -n --arg content $'# Task Status\n\n| Scope | Owner | State | Eval Round | Updated At | Notes |\n|------|------|------|-----------|-----------|------|\n| runtime-enforcement-hardening | generator | verification_check | 0 | 2026-03-29T22:00:00+0800 | - |\n\n## State Notes\n- human_ack: true\n' '{tool_input:{file_path:".harness/task-status.md",content:$content}}')"
assert_exit "human gate passes with ack" 0 run_hook "$tmp/cc-human-ok" "$human_ok_payload"

make_repo "$tmp/codex-blocked"
write_status "$tmp/codex-blocked/.harness/task-status.md" "generating" "-"
