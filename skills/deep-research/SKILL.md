...
### 6. Save findings (Standard/Deep only).

**Quick depth**: answer lives in chat. No file needed unless the user asks.

**Standard/Deep**: write findings to a file before your final chat response.
The file is the canonical artifact — write it first, then reference it in chat.

1. Find the right location: look for a `docs/` directory, a `research/`
   folder, or prior investigation notes in the project. Match existing conventions.
2. If no convention exists, use `docs/` and note the path.
3. Start the file with this header so the next investigator can assess
   ...
   **Open questions**: [count] — see end of document
   ```
4. In your chat response, confirm where the findings were saved.

If the project has no writable location (read-only codebase, no obvious
docs directory), say so and keep the answer in chat instead.
...
| **Skipping the save** (answered in chat but didn't write a file) | For Standard/Deep: the file is the artifact. Write it first, reference it in chat. |
