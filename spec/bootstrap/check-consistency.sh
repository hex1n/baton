...
if ! grep -Fq 'evaluation.template.md' "$start_task_ps1"; then
  printf 'ERROR: invariant-10: start-task.ps1 does not reset evaluation.md\n'
  inv10_errors=$((inv10_errors + 1))
fi
...
for consumer in "$validate_isolation_sh" "$harness_context_sh"; do
  if [[ -f "$consumer" ]] && ! grep -Fq 'source "$script_dir/provenance.sh"' "$consumer"; then
...
for template in "$verification_template" "$evaluation_template" "$zh_verification_template" "$zh_evaluation_template"; do
  [[ -f "$template" ]] || continue
  for needle in 'Execution Provenance' '- Role:' '- Isolation mode:' '- Execution context:' '- Evidence:' '- Fallback policy:' '- Fallback reason:'; do
...
if [[ -f "$validate_isolation_sh" ]]; then
  for needle in '"Role"' '"Isolation mode"' '"Execution context"' '"Fallback reason"'; do
...
if [[ -f "$repo_root/tests/test-validate-isolation.sh" ]]; then
  for needle in 'Role: verification_explorer' 'Role: evaluator' 'Isolation mode:'; do
...
# ---------------------------------------------------------------------------
inv13_errors=0
for hook_ref in $(grep -oE 'hooks/[A-Za-z0-9._/-]+\.sh' "$script_dir/install-hooks.sh" | sort -u); do
