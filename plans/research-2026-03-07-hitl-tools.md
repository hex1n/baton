...
→ ❌ 本次调研**未使用** context7。所有引用来源均为 web search + 直接 URL 抓取（LangGraph docs、CrewAI docs、HumanLayer GitHub、OpenAI SDK docs 等）。

Context7 适合查找**特定库的 API 文档和代码示例**（如 LangGraph 的 `interrupt()` 函数签名、CrewAI 的 `@human_feedback` 装饰器参数）。本次调研偏向**概念层和产品层**的横向比较，而非深入某个库的 API 细节，因此 context7 的价值有限。

如果后续进入**方案 B 实施阶段**（§7.3），context7 会有价值——例如查询 LangGraph checkpoint API 的具体实现模式，或 HumanLayer SDK 的 `require_approval` 配置选项。

---

**[Q] §2 Baton 与 Skill 冲突解决**
"怎么解决baton与各种skill间的冲突?"

→ 此问题已有**深度研究**，见归档文件：
- `plans/research-2026-03-06-skill-conflicts.md`（606 行，14 个 skill 逐一分析）
- `plans/plan-2026-03-06-skill-conflicts.md`（316 行，3 种方案 + 最终推荐）
- `plans/research-2026-03-06-skill-conflicts-review.md`（619 行，社区研究 + 5 轮标注）

**核心发现**：所有冲突追溯到 5 个控制点：

| 控制点 | 含义 | 高冲突 skill |
|--------|------|-------------|
| #1 计划位置/格式 | plan.md + 批注区 | writing-plans |
| #2 审批门 | `<!-- BATON:GO -->` | writing-plans |
| #3 状态跟踪 | `## Todo` + `- [ ]` 复选框 | subagent-driven-dev, executing-plans |
| #4 执行边界 | 只改 plan 列出的文件 | executing-plans |
| #5 完成流程 | Retrospective → 归档 | finishing-a-development-branch |

**推荐方案**：三层防御模型

```
Layer 1: Complexity Calibration        → 入口过滤：何时激活哪些 skill
Layer 2: Control-Point Defense Rules   → 运行时护栏：5 条通用禁令写入 CLAUDE.md
Layer 3: Pointwise Integration (可选)  → 对高价值 skill 做深度集成（如 TDD、code review）
```

**Layer 2 核心规则**（写入 CLAUDE.md，利用指令优先级 CLAUDE.md > plugin skill）：
> 当 baton workflow 激活时，任何 skill 指令与以下控制点冲突时，baton 优先：
> 1. 计划文件位置和格式遵循 baton 约定
> 2. 代码写入需要 BATON:GO
> 3. 任务跟踪使用 `## Todo` 复选框
> 4. 只修改 plan 中列出的文件
> 5. 完成流程遵循 baton 归档流程

**长期方案**：Baton skill 化——将 baton 的 phase guidance 本身做成 skill（`baton:research`、`baton:plan`、`baton:implement`），利用 using-superpowers 的 "1% 概率就必须调用" 规则作为**力量倍增器**。baton skill 因为有 hook + 人类审批的三层深度保障，在质量竞争中自然胜出。

**未实施原因**：等待人类决策——先部署 Layer 1+2（低风险），还是直接走 skill 化（高收益但工作量大）。
...
→ HOTL（Human-on-the-Loop）技术上和哲学上**与 baton 核心设计兼容**。关键是将现有的 Complexity Calibration 从"影响研究深度"扩展到"影响审批门控"。

**核心模型**：基于复杂度 + 置信度的分级审批

```
               ┌─ TRIVIAL + confidence ≥ 0.95 → 自动批准（无需 BATON:GO）
任务评估 ──→ ├─ SMALL   + confidence ≥ 0.90 → 自动批准
               ├─ MEDIUM  → 始终需要 BATON:GO + 标注周期
               └─ LARGE   → 始终需要 BATON:GO + 多轮标注
```

**write-lock.sh 变化示意**：
```sh
# 现有逻辑不变：BATON:GO 存在 → 放行
if grep -q '<!-- BATON:GO -->' "$PLAN"; then exit 0; fi

# HOTL 新增：检查复杂度 + 置信度
COMPLEXITY=$(grep -o "complexity: [A-Z]*" "$PLAN" | head -1 | cut -d' ' -f2)
case "$COMPLEXITY" in
    TRIVIAL|SMALL) # 高置信度 → 自动放行
        CONFIDENCE=$(grep -o "confidence: 0\.[0-9]*" "$PLAN" | head -1)
        [ "${CONFIDENCE:-0}" -ge 90 ] && exit 0 ;;
    *) ;; # MEDIUM/LARGE → 仍需 BATON:GO
esac
```

**不变的部分**（baton 核心优势保留）：
- ✅ Plan-first 流程（所有任务仍需 plan）
- ✅ 结构化反馈协议（Medium/Large 仍走标注周期）
- ✅ Evidence Standard（file:line 引用）
- ✅ 复杂度校准概念（扩展用途，不改变概念）
...
| 风险 | 缓解措施 |
|------|---------|
| AI 将复杂任务错标为 TRIVIAL | 复杂度评分必须写入 plan.md，人类可随时覆盖 |
| 自动批准的代码有 bug | 自动运行测试 + 每次 auto-approve 独立 git commit（易回滚） |
| 人类失去"审阅肌肉记忆" | 跟踪 auto-approval 比率，>50% 时告警 |
| 合规风险（EU AI Act） | `BATON_REGULATORY=1` 环境变量禁用自动批准 |

**分阶段路线图**：

| 阶段 | 目标 | 周期 |
|------|------|------|
| Phase 1 | 自动化复杂度评分（不改变审批门） | 2-3w |
| Phase 2 | 仅 TRIVIAL 自动批准 | 3-4w |
| Phase 3 | SMALL 也加入自动批准 | 4-6w |
| Phase 4 | 风险模式检测（DB 操作、文件删除等自动升级） | 6-8w |
| Phase 5 | 事后审阅 + 自动回滚机制 | 8-10w |

**何时该走 HOTL**：
- ✅ 团队已形成 plan-first 肌肉记忆（使用 baton 数月）
- ✅ 大部分任务是 trivial/small（UI 微调、重构、测试）
- ✅ 需要快速迭代（原型期、创业场景）
- ❌ 不适合：高合规要求、团队新手期、安全敏感代码

**建议**：当前保持 HITL 作为默认，先实施 Phase 1（自动化复杂度评分），积累数据后再决定是否开放 Phase 2。这是**零风险的第一步**。

---

### Round 2 (2026-03-07)

**[NOTE] §3 HOTL 转型路径 — 直接终极版本**
"我想直接来个终极版本"

→ 收到。不做分阶段渐进，直接设计包含所有能力的终极版本。这意味着：
- 自动化复杂度评分 + 置信度评估
- 基于复杂度的分级审批门控（Trivial/Small 自动放行）
- 风险模式检测与自动升级
- 结构化审计日志（JSON）
- 事后审阅 + 自动回滚机制
- `BATON_REGULATORY=1` 合规模式

此 NOTE 影响下一步：plan.md 将以终极版本为目标，一次性设计完整 HOTL 架构。
