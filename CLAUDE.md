...
| `plan.md` | `.harness/` | Planner + Builder (§ Discoveries) | Per task, archived on completion; Baton control-plane projection of `design.md`, carrying round classification, forecasts, `§ Plan Quality`, `§ Open Decisions`, `§ Round Contract`, and `§ Implementation Slices` |
| `review.md` | `.harness/` | Verifier | Per round, overwritten; carries `§ Routing Signals`, design-review add-on selection, pre-flight triage, verify-pass add-on selection, plan-quality assessment, confidence calibration, and round-load assessment |
...
- `Scope Class` / `Risk Class` live in `plan.md § Metadata` and classify the current round.
- `Expected Rounds` / `Expected Slices This Round` are forecasts; they can still trigger round-load warnings or overload checks.
- `Verifier Mode` describes evidence strength.
- `Execution Mode` is the orchestration choice that Dispatcher confirms from the classification.
- Complex rounds may use `Planning Depth = deepen` in `plan.md § Plan Quality` so Dispatcher can route a deeper planning pass before Builder starts.
...
- In `full` mode, Verifier pre-flight may run design-stage review add-ons before human approval; `review.md § Routing Signals` then records whether Baton should auto-revise or stop for a human checkpoint.
