...
├── phase-guide.sh         # 阶段引导（SessionStart hook）— 优先提示 skill，回退到 workflow-full.md 提取
├── stop-guard.sh          # 停止提醒（Stop hook）
├── bash-guard.sh          # Bash 写入警告（PreToolUse Bash hook）
├── workflow.md            # 核心规则（~400 tokens，始终加载）
├── workflow-full.md       # 完整参考（skill 不可用时的 fallback 源）
└── adapters/              # 跨 IDE 适配器（不变）

.claude/skills/             # 阶段方法论（agentskills.io 标准）
├── baton-research/SKILL.md # 研究阶段 skill
├── baton-plan/SKILL.md     # 计划+标注阶段 skill
└── baton-implement/SKILL.md# 实现阶段 skill
```
...
### 标注协议
人在 research.md 或 plan.md 中加标注，AI 逐条回应，记入 ## Annotation Log：
- `[NOTE]` 补充信息 → 纳入，说明如何影响结论
- `[Q]` 提问 → 用 file:line 证据解释
- `[CHANGE]` 要求修改 → 如果有问题，用证据说明 + 给替代方案，让人决定
- `[DEEPER]` 不够深 → 继续调查指定方向
- `[MISSING]` 遗漏 → 补充调查
- `[RESEARCH-GAP]` 需要更多调研 → 暂停当前文档修改，补充研究后回来
...
人用 [DEEPER]/[MISSING]/[WRONG]/[Q] 标注 → AI 回应并更新 → 循环直到人满意
```
...
例如：
    ...
    ### 设计方案：使用 Service 层校验
    [Q] 为什么不在 middleware 统一做？这样每个 service 都要重复
    [NOTE] 历史上 middleware 做过校验但因为性能问题移到了 service 层
...
4. 回到当前文档，继续处理剩余标注
5. 在 Annotation Log 中记录：gap 内容 + 关键发现 + 对当前文档的影响
```
...
已安装时提示用户调用 skill（如 `/baton-research`），未安装时回退到 workflow-full.md 提取或硬编码摘要。
...
3. 实现阶段
   ...
   → 无 skill：从 workflow-full.md 提取 IMPLEMENT 引导

4. 标注循环（plan）
   plan.md 存在 + 无 BATON:GO
   → 有 baton-plan skill：提示 /baton-plan
   → 无 skill：从 workflow-full.md 提取 ANNOTATION 引导

5. 计划阶段
   research.md 存在 + 无 plan.md
   → 有 baton-plan skill：提示 /baton-plan
   → 无 skill：从 workflow-full.md 提取 PLAN 引导

6. 研究阶段
   无 research.md + 无 plan.md
   → 有 baton-research skill：提示 /baton-research
   → 无 skill：从 workflow-full.md 提取 RESEARCH 引导
...
**无 skill 时**（回退到 workflow-full.md 提取或硬编码摘要）：
```
...
1. **版本号更新**：v2.0 → v3.0
2. **安装的文件更新**：workflow.md 和 workflow-full.md 内容更新
3. **v2 → v3 迁移**：
   - 检测 v2 安装 → 原地升级（替换 workflow.md / workflow-full.md / phase-guide.sh / stop-guard.sh）
...
| 3 | 有 plan.md，无 GO → ANNOTATION 引导 | 包含"标注循环"、"[NOTE]"、"[Q]"、"人不一定正确" |
...
验证 workflow.md 和 workflow-full.md 的共享段落一致性（不变，但匹配新内容）。

### 新增：test-annotation-protocol.sh

这不是 hook 测试，而是文档一致性测试：
- workflow.md 中列出的标注类型和 workflow-full.md 一致
- workflow-full.md 中每种标注类型有示例
...
2. 替换文件：workflow.md、workflow-full.md、phase-guide.sh、stop-guard.sh
...
baton_bak 的用户：
- 标注类型 [NOTE]/[Q]/[CHANGE]/[RESEARCH-GAP] 保持一致
- 新增 [DEEPER]/[MISSING] 用于 research 标注
- 不再需要 CLI（bin/baton）
...
| 组件 | 改动量 | 说明 |
|------|-------|------|
| workflow.md | 重写 | 标注协议 + 两种场景 + 规则 |
| workflow-full.md | 重写 | 详细标注说明 + 示例 + 各阶段引导 |
| phase-guide.sh | 改造 | 增加 research.md 检测 + 标注循环提示 + 归档提醒 |
...
| ~400 token 开销 | ✅ | workflow.md ~380 tokens |
| 零依赖 | ✅ | 纯 shell + markdown |
| 标注循环结构化 | ✅ | 6 种标注类型 + Annotation Log |
