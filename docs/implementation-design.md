...
```
📍 研究阶段 — 产出 research.md
深入阅读代码，追踪调用链到实现层（不要停在接口）。
标记风险：✅ 已确认安全 / ❌ 发现问题 / ❓ 待确认。每个结论附 file:line。
简单改动可跳过研究，直接写 plan.md。
```

**PLAN 阶段**：
```
📍 计划阶段 — 产出 plan.md（基于 research.md + 需求）
包含：做什么（引用 research）、为什么、影响范围、风险应对。
...
```
📍 标注循环阶段 — plan.md 等待审批
人可能在文档中添加标注：[NOTE] [Q] [CHANGE] [DEEPER] [MISSING] [RESEARCH-GAP]
收到标注后逐条回应，记录到 Annotation Log。
人的标注不一定正确 — 有问题用 file:line 证据说明，给替代方案。
...
```
📍 实现阶段 — <!-- BATON:GO --> 已添加
按 Todo 顺序实现。每个 item 完成后 typecheck → 标记 [x]。
全部完成后跑完整测试。发现遗漏 → 停下更新 plan，等人确认。
```
...
💡 Annotation Log 记录了设计决策理由，有长期参考价值。
```
