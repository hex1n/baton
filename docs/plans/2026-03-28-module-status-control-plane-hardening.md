...
**Goal:** Close the concrete P0 gap identified in `docs/review-analysis.md` by making `task-status.md` a stable control plane with one shared row-selection contract across bootstrap, hooks, session context, validators, and status/reporting surfaces.
...
**Key decision:** Define the **current task row** as the **last data row** in the `task-status.md` task table. All readers and writers must use that same rule. Legacy 5-column rows must still be readable during migration, with `Eval Round = 0` as the compatibility default.

---

## Workstream 1: Contract

- Add one shared parsing implementation for `task-status.md`.
...
- Fix `start-task.sh` so it can read legacy live `task-status.md` files without dropping history rows.
...
- Extend `check-consistency.sh` to validate the live `.harness/task-status.md` shape when present.
...
- live `task-status.md` is migrated to the new schema
